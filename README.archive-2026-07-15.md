...
前端默认通过同源 `/api` 访问后端。如需覆盖 API 地址，可在项目根目录复制前端模板：
...
详细配置说明请参考 `server/.env.example`；兼容旧部署流程的
`server/env.template` 与它保持一致。
...
```bash
pm2 start src/index.js --name amazon-asin-monitor --cwd server
```
...
│   ├── env.template       # 兼容旧部署流程的同内容模板
│   └── index.js           # 入口文件
├── dist/                  # 前端构建输出（生产环境）
├── scripts/               # 脚本文件
├── .env.example           # 前端构建环境变量示例
...
如果历史功能分支对应的 PR 已经互相冲突，且团队决定以当前本地分支代码作为最终状态，请不要继续把这些旧分支逐个合并到主分支。推荐流程：

1. 从当前本地分支创建一个新的整合 PR，并在 PR 描述中说明“以当前本地代码为准”。
2. 对仍指向旧功能分支、且内容已包含在当前本地代码中的 PR，统一关闭并备注已由整合 PR 取代。
3. 关闭旧 PR 后删除对应远端分支，避免 GitHub 继续提示 `Compare & pull request`。
4. 后续新改动从最新主分支重新拉分支，避免继续基于旧冲突分支开发。
