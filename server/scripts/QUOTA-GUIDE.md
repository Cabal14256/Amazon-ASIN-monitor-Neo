...
- 从主营库读取 `sp_api_config`，合法数据库值优先于同名环境变量；数据库值为空、`null` 或非法时回退环境变量，环境变量也无效时才使用程序默认值；
- 只统计已经属于监控变体组的 ASIN，不把未分组记录算入定时任务；
- 在竞品监控开启时，通过 `COMPETITOR_DB_*` 独立连接竞品库；连接或查询失败会以非零状态退出，不输出不完整结论；
- 按 US、EU 分别报告主营与竞品数量、完整轮转时间、operation 请求数、平均负载、峰值批次、峰值滚动小时和本地区域上限占用；
...
平均负载用于观察长期容量，限额判断使用实际 CRC32 分桶后的峰值。峰值批次按单次调度中请求数最多的批次计算；峰值滚动小时按调度顺序计算任意一小时内连续批次的最大请求和。例如三批请求量为 `1000/700/300` 且每 30 分钟执行一批时，峰值批次为 `1000`，峰值滚动小时为 `1700`，不会再用平均批次 `667` 判断是否安全。
...
| `SP_API_RATE_LIMIT_BURST_CAP` | operation 每秒突发上限；独立于持续速率安全系数，且不超过已知 operation burst |
