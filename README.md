...
Neo 角色列表、详情、权限分组、权限分配及角色下拉接口已接入 PostgreSQL，权威源门禁、并发权限保护和缓存失效边界见 [`角色管理运行说明`](./docs/runbooks/phase-2-role-management.md)。用户列表与详情见 [`用户查询运行说明`](./docs/runbooks/phase-2-user-queries.md)，创建、更新与删除见 [`用户管理运行说明`](./docs/runbooks/phase-2-user-administration.md)；管理员密码重置仍待迁移。
