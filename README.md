...
Timescale 索引与存储 Gate 会在 72 万次确定性高频监控 fixture 上生成脱敏的 `artifacts/timescale-performance/integration-report.json`：验证 7 个原始历史索引的真实执行计划、先转入 columnstore 的 9 个 CAGG 在冷热窗口/筛选/hour-day-month 下与 raw 结果一致且 P95 至少快 3 倍，以及 columnstore 晚到写入和 10 批共 2500 行持续写入期间的分析读取。索引取舍见 [`packages/db/INDEX_REVIEW.md`](./packages/db/INDEX_REVIEW.md)，上线与回滚见 [`docs/runbooks/phase-1-timescale-storage.md`](./docs/runbooks/phase-1-timescale-storage.md)。
