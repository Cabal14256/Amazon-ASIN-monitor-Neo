...
每次审计写入使用独占连接和局部事务，SQL 超时 1500ms、连接获取及写入合计硬截止 2 秒；超时会销毁该连接并停止后续 SQL，晚到的连接只释放、不执行审计。这样卡住的审计不会在 flush 返回后继续拖住池关闭，且超时设置不影响共享池的分析/导出查询。超时可能丢弃本次审计记录，由上述错误码日志告警。
