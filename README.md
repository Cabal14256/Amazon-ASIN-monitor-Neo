...
Neo 共享 SP-API 客户端的已实现范围和未完成接入门槛见 [`P2-T3 运行说明`](./docs/runbooks/phase-2-spapi-client.md)，共享原子配额、优先级执行与故障回退见[配额运行说明](./docs/runbooks/phase-2-spapi-quota.md)，三个受权限保护的配置接口与 PostgreSQL 配置来源见[配置运行说明](./docs/runbooks/phase-2-spapi-config.md)，进程内错误统计和风险阈值见[指标运行说明](./docs/runbooks/phase-2-spapi-telemetry.md)，默认关闭的 HTML 商品关系兜底见[HTML 运行说明](./docs/runbooks/phase-2-spapi-html.md)，共享简化备用请求见[Legacy 客户端说明](./docs/runbooks/phase-2-spapi-legacy.md)，API 单例及受权限保护的状态接口见[Runtime 说明](./docs/runbooks/phase-2-spapi-runtime.md)；目前不替代 Legacy 的生产调用。
...
D4 会话清理与审计归档见[数据层说明](./docs/runbooks/phase-2-auth-maintenance-data.md)和[Worker 调度运行说明](./docs/runbooks/phase-2-auth-maintenance-worker.md)。新审计查询依赖 `0003` 冷热统一视图，最终快照导入后须先执行 `corepack pnpm db:upgrade:auth-maintenance` 再部署 API；维护专用 Worker 使用 `WORKER_ENABLED_QUEUES=maintenance`，停止已有计划需按运行说明显式暂停队列并删除计划。
...
Neo API 的全局审计拦截器沿用 Legacy 的认证、用户、角色、ASIN/变体组、配置、导出与手动监控动作映射；在响应完成后记录最终 HTTP 状态，普通只读请求、OPTIONS/HEAD 和未知路由跳过。审计写入 PostgreSQL 主库 `audit_logs`，不依赖 MySQL 鉴权用户在 PostgreSQL 中存在副本。四个只读审计端点已接入 PostgreSQL，见 [`审计查询运行说明`](./docs/runbooks/phase-2-audit-query.md)；归档调度尚待迁移，双跑期 Legacy 的新增审计仍由旧系统提供。
...
配置值 `configValue`/`config_value`/`displayValue` 一律掩码，不依赖配置键是否命中已知凭据名；请求体的 `real_name`/`realName`、`statusReason`、邮箱、电话等个人字段递归脱敏，`sessionId`/`session_id`/`sessionIds`/`sid` 等会话标识同样掩码。Neo 创建 Fastify 时消费 `TRUST_PROXY`：默认不信任转发头；单层受信 Nginx 可设置 `1`，更推荐按代理地址/CIDR 限定（如 `loopback`），并确保 API 端口不能绕过代理对外访问。未受信的直接连接不能通过 `X-Forwarded-For` 覆盖审计 IP。

响应正常完成时记录最终 HTTP 状态；客户端中途断开先标记传输状态，待认证 Guard 和处理器结束后才持久化最终操作人，正常处理但响应未送达记为审计状态 499，认证/业务失败保留已确定的错误 HTTP 状态，响应流异常记为 500 与固定失败标记。客户端 IP 在请求开始时快照，避免断连后 socket 地址消失。响应关闭、流错误和业务完成的多个回调共享一次性记录保护，不重复写入；这些是传输结果，不代表已经开始的数据库修改被回滚。
...
每次审计写入使用独占连接和局部事务，SQL 超时 1500ms、连接获取及写入合计硬截止 2 秒；超时会销毁已开始执行审计的连接并停止后续 SQL，晚到且未使用的健康连接正常归还共享池、不执行审计。这样卡住的审计不会在 flush 返回后继续拖住池关闭，且超时设置不影响共享池的分析/导出查询。超时可能丢弃本次审计记录，由上述错误码日志告警。

总共 256 条待写记录包含活动与排队项，实际同时只启动 2 条审计写入，避免突发审计占满共享的 10 个主库连接。仓库层也保持最多 2 个真实连接获取/写入名额：即使调用方已收到 2 秒超时，未结束的连接获取仍持有名额，不会继续堆积隐藏的共享池等待者。关闭的 5 秒期限到达时丢弃未启动项并记录数量，已开始的操作沿用自身截止机制，池关闭后不再启动排队 SQL。
...
PostgreSQL 权威源下已注册认证维护、主营批量删除和导入的真实 Processor；选择 `batch-delete` 或 `import` 后主入口显示 `mode=business-worker` 与实际注册数。主营批量删除支持同步事务、异步分块、任务查询及取消，详见[批量删除运行说明](./docs/runbooks/phase-2-asin-batch-delete.md)。主营 CSV/XLSX 导入使用共享持久化文件目录、流式解析及分块写入，完整结果可由任务所有者下载；启用前需完成主库 0005 升级，详见[文件导入运行说明](./docs/runbooks/phase-2-asin-import.md)。其余六类业务消费者、竞品处理和跨进程 WS 事件桥继续迁移，未切换生产任务。共享队列策略、变量和新旧命名空间纪律见 [P2-T2a 运行说明](./docs/runbooks/phase-2-bullmq-queues.md)。
