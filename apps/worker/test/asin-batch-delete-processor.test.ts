...
} from '../src/asin-batch-delete-processor';
...
function fixture(domain: 'asin' | 'competitor' = 'asin') {
  ...
  const other = {
    transaction: vi.fn(async () => {
      throw new Error('Wrong business database');
    }),
  };
  const dispatcher = createBatchDeleteProcessor(
    domain === 'asin'
      ? { asin: repository, competitor: other }
      : { asin: other, competitor: repository },
    store,
    options,
    log,
  );
  ...
  return {
    ...
    run: () =>
      domain === 'asin'
        ? processor(job, 'fixture-lock')
        : dispatcher(job, 'fixture-lock'),
    dispatch: () => dispatcher(job, 'fixture-lock'),
  };
}
describe.each(['asin', 'competitor'] as const)(
  'actual %s batch processor control flow',
  (domain) => {
    const fixtureForDomain = () => fixture(domain);
    it('dispatches to exactly the matching business database', async () => {
      const f = fixtureForDomain();
      expect(await f.dispatch()).toMatchObject({ verificationPassed: true });
    ...
    it('rejects changing a complete domain identity against stored task metadata', async () => {
      ...
      f.job.name = `${otherDomain}-batch-delete`;
      Object.assign(f.job.data, {
        domain: otherDomain,
        taskSubType:
          otherDomain === 'asin'
            ? 'variant-group-delete'
            : 'competitor-variant-group-delete',
        title: otherDomain === 'asin' ? '批量删除变体组' : '批量删除竞品变体组',
      });
      await expect(f.dispatch()).rejects.toThrow('批量删除失败');
      expect(f.repository.transaction).not.toHaveBeenCalled();
      expect(f.other.transaction).not.toHaveBeenCalled();
      expect(f.store.mutate).not.toHaveBeenCalled();
    });
    it('rejects a queue job name inconsistent with its domain', async () => {
      const f = fixtureForDomain();
      f.job.name =
        domain === 'asin' ? 'competitor-batch-delete' : 'asin-batch-delete';
      await expect(f.dispatch()).rejects.toThrow('任务数据无效');
    ...
    it('cancellation during a chunk keeps committed deletion and stops subsequent chunks', async () => {
      ...
      f.options.updateProgress.mockImplementation(async (_job, progress) => {
        if (progress > 5)
          f.patch({
            status: 'cancelling',
            cancelRequestedAt: f.data.createdAt,
          });
      });
      expect(await f.run()).toMatchObject({ cancelled: true });
    ...
    it.each(['completed', 'cancelled', 'failed'] as const)(
      ...
      async (status) => {
        ...
        if (status === 'failed')
          await expect(f.run()).rejects.toThrow('已失败');
        else await f.run();
        expect(f.repository.transaction).not.toHaveBeenCalled();
        expect(f.store.mutate).not.toHaveBeenCalled();
