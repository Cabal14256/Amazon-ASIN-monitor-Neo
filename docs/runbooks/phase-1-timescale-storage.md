...
4. 执行完整 Integration；下载 `timescale-performance-<run_id>` artifact，确认 7 份执行计划、12 组摘要一致、每组 CAGG P95 至少快 3 倍、列存晚到写入与并发读取通过。
5. 检查没有长事务或 DDL，确认 30 秒锁等待失败会被监控捕获。`transaction_per_chunk` 降低索引构建的长事务风险，但每个 chunk 仍会短时持锁。
