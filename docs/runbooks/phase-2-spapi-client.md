...
3. 真实 BullMQ Processor/调度与监控 pipeline 接线，HTML fallback、风险判断和通知对拍。Issue #77 已提供[默认关闭的有界 HTML 客户端](./phase-2-spapi-html.md)，开关和回退管线仍待宿主接线。Issue #75 已提供[共享错误统计和风险指标](./phase-2-spapi-telemetry.md)；宿主仍须明确检查/尝试统计单位并应用并发建议。
