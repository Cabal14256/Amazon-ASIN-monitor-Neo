...
已合入主线的全局审计接线会在响应完成后记录 LOGIN：只有事务成功返回后才设置受信操作人；密码错误、账号锁定或事务失败的记录保留提交的用户名，但 `user_id` 为 null。请求密码始终掩码，不记录响应 Token、Session ID 或底层异常文本。审计是独立的尽力写入，不属于登录事务；一次审计缺失不代表登录未提交。

真实登录联调同时发现，脱敏器的无原型对象触发 Drizzle 0.44 的实体检测异常，导致带请求体的审计尚未发送 SQL 就失败。审计仓储现将脱敏后的 JSON 序列化为独立参数并显式绑定为 jsonb，保留脱敏及特殊键，不拼接请求内容到 SQL。回归验证无原型对象、嵌套内容和 constructor/**proto** 键；集成测试同时检查审计接收、持久化失败日志与最终记录，避免将 HTTP 成功当成审计成功。
...
Integration CI 使用其一次性 PostgreSQL/TimescaleDB 服务执行：验证大小写用户名登录及新 Token 访问 current-user、记住登录与过期标记、五个并发错误密码的原子计数、插入 Session 后失败时全部回滚、真实行锁等待超时且不产生 Session/Cookie。同时加载实际 AuthModule 与 AuditModule，验证成功、并发失败/锁定和回滚记录的最终身份、状态、恰好一次及脱敏；HTTP 单测额外暂停事务提交，确认提交完成前没有审计身份泄露。fixture 在审计 flush 后只清理本次随机前缀用户、关联 Session、登录尝试及对应 LOGIN 审计记录，不修改业务账号。CI 验收不能替代生产规模性能、最终数据同步或灰度验证。
