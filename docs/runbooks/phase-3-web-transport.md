...
关联 Issue #35。本批建立通信基础设施；后续 Issue #55 已接入 [认证状态、15 路由及账号页面](phase-3-auth-pages.md)，尚不构成全部业务页面或 P3 阶段验收。
...
- 普通 403 保持权限错误，不清空全局登录；登录接口 401 不触发“旧会话过期”。其他 HTTP/信封 401 清本地提示、旧 WS/Query/在途请求并通知宿主。Issue #55 由 auth context/guard 按当前路径生成安全的登录跳转，同时处理 bootstrap 403、强制改密及跨标签页校验。
